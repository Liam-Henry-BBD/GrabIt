...
export class DashboardComponent extends LitElement {
        ...
        const createProjectComponent = (project: Project): TemplateResult => {
            return html`
                <li class="project-item">
                    ${project.collaboratorRole == 1
                        ? html`<button class="project-del" @click=${() => this.showDeletePopup(project)}>-</button>`
                        : html`<span>📁</span>`}
    ...
    render() {
        return html`
                        ...
                        (Object.keys(this.projectOrganiser) as (keyof ProjectOrganizer)[]).map(group => {
                            return html`
                                <h2 class="project-header">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-folder-plus mr-2 h-4 w-4 text-[#F7F0F0]/70">
            ...
            ${this.isDeletePopupVisible ? html`
